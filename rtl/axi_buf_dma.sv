...
module axi_buf_dma #(
  ...
  always_comb begin
    ...
    case (state)
      DMA_W_AW: begin
        wait_state = 1'b1;
        forward_progress = m_axi_awready;
      end
      DMA_W_SEND: begin
        wait_state = 1'b1;
        forward_progress = m_axi_wready;
      end
      DMA_W_RESP: begin
        wait_state = 1'b1;
        forward_progress = m_axi_bvalid;
      end
      DMA_R_AR: begin
        wait_state = 1'b1;
        forward_progress = m_axi_arready;
      end
      DMA_R_DATA: begin
        wait_state = 1'b1;
        forward_progress = m_axi_rvalid;
      end
      default: begin
        wait_state = 1'b0;
        forward_progress = 1'b0;
  ...
  always_ff @(posedge clk) begin
    if (!rst_n) begin
      timeout_count <= 0;
    end else if (!wait_state || forward_progress) begin
      timeout_count <= 0;
  ...
  always_ff @(posedge clk) begin
    if (!rst_n) begin
      buf_dout <= 32'b0;
    end else if (!dma_busy && !buf_csn && !buf_wr_en) begin
          ...
          DMA_W_RESP: begin
            if (m_axi_bvalid) begin
              if (m_axi_bresp != AXI_RESP_OKAY) begin
                ...
                state          <= DMA_ERROR;
              end else if (remaining_words == 0) begin
                state <= DMA_DONE;
              end else begin
